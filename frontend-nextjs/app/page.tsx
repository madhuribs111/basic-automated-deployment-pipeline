...
// IMPORTANT: Socket server runs on port 9002
...
export default function Home() {
  ...
  const handleClickDeploy = useCallback(async () => {
    ...
    try {
      ...
      if (data && data.projectId && data.url) {
        const { projectId: slug, url } = data;

        setProjectId(slug);
        setDeployPreviewURL(url);

        const logChannel = `logs:${slug}`;
        console.log(`Subscribing to ${logChannel}`);
        socket.emit("subscribe", logChannel);
      }
    } catch (err) {
  ...
  const handleSocketIncommingMessage = useCallback((message: string) => {
    try {
      ...
      console.log("Non-JSON socket message:", message);
